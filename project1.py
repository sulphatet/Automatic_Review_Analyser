...
def hinge_loss_full(feature_matrix, labels, theta, theta_0):
    ...
    """
    # Your code here
    #print(len(feature_matrix)==len(labels))
    Z = []
    for i in range(len(feature_matrix)):
        Z.append(hinge_loss_single(feature_matrix[i], labels[i], theta, theta_0))
    return sum(Z)/len(Z)
...
def perceptron_single_step_update(
    ...
    if (np.dot(feature_vector,current_theta) + current_theta_0) * label <= 0:
        current_theta += np.dot(feature_vector,label)
...
def average_perceptron(feature_matrix, labels, T):
    ...
    for t in range(T):
        ...
        for i in get_order(feature_matrix.shape[0]):
            ...
            perceptron_single_step_update(feature_matrix[i,:], labels[i], current_theta, current_theta_0)
...
def classify(feature_matrix, theta, theta_0):
    ...
    for i in range(feature_matrix.shape[0]):
        Z = np.dot(feature_matrix[i],theta) + theta_0
        if Z <= 0:
            labels[i] = -1
    return labels
...
def classifier_accuracy(
    ...
    y_pred = classify(val_feature_matrix, theta, theta_0)
    y_trained = classify(train_feature_matrix,theta,theta_0)
    ac_trained = accuracy(y_trained, train_labels)
    ac_val = accuracy(y_pred, val_labels)
    return (ac_trained,ac_val)
    ...
    for i, text in enumerate(reviews):
        ...
        for word in word_list:
            if word in dictionary:
                ...
                #feature_matrix[i, dictionary[word]] += 1 #returns a lower accuracy
